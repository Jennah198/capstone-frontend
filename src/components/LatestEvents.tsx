...
interface Event {
  ...
  vipPrice?: TicketPrice;
  isPublished: boolean;
  description?: string;
  capacity?: number;
}

type EventStatus = 'draft' | 'upcoming' | 'ongoing' | 'completed' | 'published';
type FilterType = 'upcoming' | 'all' | 'trending';
...
const LatestEvents: React.FC = () => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        let eventsData: Event[] = response.data.data || [];
      ...
      if (err.response) {
        if (err.response.status === 401 || err.response.status === 403) {
          // clear user state instead of forcing immediate navigation to /login
          setUser(null);
        }
      } else {
  ...
  const getStatusBadge = (event: Event): StatusBadge => {
    const status = getEventStatus(event);
    const badges: Record<EventStatus, StatusBadge> = {
                    ...
                    {event?.image ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          target.onerror = null;
                          target.src = 'https://images.unsplash.com/photo-1511578314322-379afb476865?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
                        }}
                      />
                    ) : (
