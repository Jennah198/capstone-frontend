  ...
  return (
    <div className=''>
      <div className='max-w-6xl mx-auto py-14 px-3 lg:px-0'>
        ...
        <div 
          ref={scrollRef} 
          className='category-container w-full flex gap-3 overflow-x-scroll'
        >
          {categories && categories.length ? 
            categories.map((cat) => (
              <div key={cat._id} className='relative rounded-sm overflow-hidden flex-none'>
                <img 
                ...
                <div className='absolute inset-0 bg-linear-to-t from-black/80 to-transparent flex items-center justify-center'>
                  ...
                  <Link 
                    ...
                    className='text-white border text-sm absolute bottom-5 right-6 cursor-pointer px-3 rounded-full'
                  >
                    View
                  </Link>
                </div>
