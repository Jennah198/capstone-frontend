...
type EventStatus = 'draft' | 'published' | 'upcoming' | 'ongoing' | 'completed';
type FilterType = 'all' | 'draft' | 'published' | 'upcoming' | 'ongoing' | 'completed';
...
const AdminEventListPage: React.FC = () => {
  ...
  const handleTogglePublish = async (eventId: string, currentStatus: boolean) => {
    try {
      ...
      if (response.data.success) {
        ...
        const msg = response.data.isPublished ? 'Event published Successfully' : 'Event Unpublished Successfully';
        toastSuccess(msg);
      }
    } catch (error) {
  ...
  const getStatusBadge = (status: EventStatus): StatusBadge => {
                    ...
                    <div className="grid grid-cols-12 gap-4 items-center">
                      ...
                      <div className="col-span-3">
                        <div className="space-y-1">
                          ...
                          <div className="space-y-1">
                            {event.normalPrice?.price && event.normalPrice.price > 0 && (
                              <div className="flex items-center justify-between text-sm">
                                <span className="text-gray-600">Normal:</span>
                                <span className="font-medium">{formatCurrency(event.normalPrice.price)}</span>
                              </div>
                            )}
                            {event.vipPrice?.price && event.vipPrice.price > 0 && (
                              <div className="flex items-center justify-between text-sm">
