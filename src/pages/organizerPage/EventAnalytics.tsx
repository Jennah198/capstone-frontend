...
const OrganizerDashboard: React.FC = () => {
  ...
  const [timeRange, setTimeRange] = useState<string>('month'); // month, week, year
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const events: Event[] = response.data.data || response.data.events || [];
  ...
  const getRecentEvents = (events: Event[]): RecentEvent[] => {
    return events
  ...
  const getTopEvents = (events: Event[]): TopEvent[] => {
    return events
      .map(event => ({
        id: event._id || event.id || '',
        title: event.title,
  ...
  const calculateRevenueData = (events: Event[]): RevenueData[] => {
  ...
  const calculateTicketsSold = (event: Event): number => {
